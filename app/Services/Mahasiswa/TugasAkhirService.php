...
class TugasAkhirService
{
    ...
    {
        $tugasAkhir->loadMissing('peranDosenTa');
        ...
        DB::transaction(function () use ($tugasAkhir, $file, $tipeDokumen, $catatan, $pembimbing1, $pembimbing2, $mahasiswaId) {
            ...
            // Langkah 1: Simpan file fisik ke storage.
            $filePath = $file->store("dokumen_ta/{$tugasAkhir->id}", 'public');
            $originalName = $file->getClientOriginalName();

            // Langkah 2: Buat record di tabel 'dokumen_ta'.
            // ✅ PERBAIKAN: Tambahkan 'file_path' di sini untuk memenuhi aturan database lama.
            $dokumenTa = $tugasAkhir->dokumenTa()->create([
                'tipe_dokumen'  => $tipeDokumen,
                'version'       => ($tugasAkhir->dokumenTa()->where('tipe_dokumen', $tipeDokumen)->max('version') ?? 0) + 1,
                'file_path'     => $filePath, // <-- KUNCI PERBAIKAN
            ]);

            // Langkah 3: Buat record di tabel 'file_uploads' untuk arsitektur baru.
            $dokumenTa->files()->create([
                'file_path'     => $filePath,
                'original_name' => $originalName,
                'file_type'     => $tipeDokumen,
            ]);

            // --- LOGIKA SESI BIMBINGAN DAN CATATAN (TETAP SAMA) ---
            ...
            if ($sesiAktif->isNotEmpty()) {
                ...
                $sesiBimbinganP2 = $pembimbing2 ? $sesiAktif->where('dosen_id', $pembimbing2->dosen_id)->first() : null;
            } else {
                $sesiKe = ($tugasAkhir->bimbinganTa()->where('status_bimbingan', 'selesai')->distinct('sesi_ke')->count()) + 1;
                $dataBimbingan = ['sesi_ke' => $sesiKe, 'status_bimbingan' => 'diajukan'];
